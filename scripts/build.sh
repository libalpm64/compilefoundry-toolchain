...
sed -i 's/var wasmBinary;/var wasmBinary=Module["wasmBinary"];/' "$DIST/clang.js" "$DIST/lld.js"
sed -i 's/function getBinarySync(file){if(readBinary)/function getBinarySync(file){if(wasmBinary)return wasmBinary;if(readBinary)/' "$DIST/clang.js" "$DIST/lld.js"
grep -q 'var wasmBinary=Module\["wasmBinary"\];' "$DIST/clang.js"
grep -q 'var wasmBinary=Module\["wasmBinary"\];' "$DIST/lld.js"
grep -q 'function getBinarySync(file){if(wasmBinary)return wasmBinary;' "$DIST/clang.js"
grep -q 'function getBinarySync(file){if(wasmBinary)return wasmBinary;' "$DIST/lld.js"
...
cp "$ROOT/compat/include/semaphore" "$BUILD/sysroot/include/c++/v1/semaphore"
...
for MODE in eh noeh; do
    ...
    cp "$ROOT/compat/include/semaphore" "$BUILD/sysroot/include/wasm32-wasip1/$MODE/c++/v1/semaphore"
